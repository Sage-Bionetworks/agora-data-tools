...
def melt_wide_dataframe(
    ...
    """Reshape a wide assay matrix into long form, splitting combined column headers.

    A wide matrix has one row per sample and one column per measured feature, where each
    feature column name encodes multiple attributes joined by a separator (for proteomics
    the header is gene_symbol|uniprotid). This helper melts the feature columns into rows
    and splits each header into the attribute columns named by header_names, so future wide
    assay transforms can reuse the same reshaping logic.

    Args:
        df: Wide DataFrame containing id_vars plus one value column per feature.
        id_vars: Columns that identify each sample and are preserved as-is (not melted),
            e.g. specimenid and individualid.
        header_names: Names to assign, in order, to the pieces produced by splitting each
            feature column header on sep. When a header has fewer pieces than expected, the
            missing trailing pieces become empty strings.
        value_name: Name of the column holding the melted measurement values.
        sep: Separator used inside each feature column header.
        dropna: If True, drop rows whose value is null (features not measured in a sample).

    Returns:
        Long-form DataFrame with columns id_vars + header_names + [value_name].

    Raises:
        ValueError: If any id_vars column is missing from df, or header_names is empty.
    """
    if not header_names:
        raise ValueError("melt_wide_dataframe: header_names must not be empty.")

    missing = [col for col in id_vars if col not in df.columns]
    if missing:
        raise ValueError(
            f"melt_wide_dataframe: id_vars columns not found in DataFrame: {missing}"
        )

    header_col = "__wide_header__"
    long_df = df.melt(id_vars=id_vars, var_name=header_col, value_name=value_name)

    # Split each combined header into at most len(header_names) pieces. Using n limits the
    # number of splits so a separator appearing inside the final piece is preserved.
    split = (
        long_df[header_col]
        .astype(str)
        .str.split(sep, n=len(header_names) - 1, expand=True)
    )

    # Headers with fewer pieces than expected leave trailing split columns absent; add them
    # so every header_name maps to a column.
    for position in range(split.shape[1], len(header_names)):
        split[position] = ""
    split = split.fillna("")

    for position, name in enumerate(header_names):
        long_df[name] = split[position]

    long_df = long_df.drop(columns=[header_col])
...
def bucket_age(
    ...
    """Bucket continuous ages into nominal age-group labels.

    Thin wrapper around pandas cut that keeps the bin edges and labels in the caller so the
    thresholds live in one place and are easy to correct. With right=True (the default) each
    interval is closed on the right, so a value equal to a boundary falls into the lower
    bucket (e.g. an edge of 6 puts age 6 in the bucket ending at 6).

    Args:
        ages: Numeric series of ages to bucket.
        bins: Monotonically increasing bin edges. Length must be len(labels) + 1.
        labels: Label assigned to each interval between consecutive edges.
        right: Whether intervals are closed on the right (value <= edge).

    Returns:
        Object-dtype series of age-group labels. Ages that fall outside every bin become
        None.
    """
    bucketed = pd.cut(ages, bins=bins, labels=labels, right=right).astype("object")
    return bucketed.where(bucketed.notna(), None)
