...
def transform_model_details(
    ...
    biomarkers_df['tissue'] = biomarkers_df['tissue'].apply(lambda x: ' '.join(word.capitalize() for word in x.split()))
    ...
    for _, model_row in model_info_df.iterrows():
        ...
        for _, allele_row in model_alleles.iterrows():
            ...
            if allele_row['mgi_allele_id'] in human_transgene_allele_map_df['mgi_allele_id'].values:
                matching_row = human_transgene_allele_map_df[human_transgene_allele_map_df['mgi_allele_id'] == allele_row['mgi_allele_id']]
                if len(matching_row) > 0 and matching_row.iloc[0]['gene_symbol'] == allele_row['gene']:
        ...
        for (evidence_type, tissue, age), group in model_biomarkers_df.groupby(['evidence_type', 'tissue', 'age_death']):
            biomarker_entry = {
                'model': model_name,
        ...
        for (evidence_type, tissue, age), group in model_pathology_df.groupby(['evidence_type', 'tissue', 'age_death']):
            pathology_entry = {
                'model': model_name,
                'evidence_type': evidence_type,
                'tissue': tissue,
