...
keeps <- c('specimenID', 'individualID', 'model', 'type', 'measurement', 'units', 'ageDeath', 'tissue', 'sex', 'genotype')
...
download_file <- function(synId, df_name, type) {
  ...
  input_dir <- file.path("../../../staging/", "input")
...
pathology_merged <- bind_rows(
  threex_final %>% mutate(across(individualID, as.character)),
  fivex_final %>% mutate(across(individualID, as.character)),
  nss_final %>% mutate(across(individualID, as.character)),
  abca7_final %>% mutate(across(individualID, as.character))
) %>%
  rename(
...
# write csv file
output_dir <- file.path("../../../staging/", "output")
