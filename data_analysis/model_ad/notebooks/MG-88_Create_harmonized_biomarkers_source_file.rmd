...
results %>% pivot_longer(
       ...
       #
       ) %>% dplyr::mutate(type = if_else(type %in% c("Abeta40", "Abeta42"), paste(fraction_type, type), type)
...
threex_abs_sol <- threex_abs_sol_source %>% dplyr::mutate(fraction_type = 'Soluble')
threex_abs_insol <- threex_abs_insol_source %>% dplyr::mutate(fraction_type = 'Insoluble')
...
# Abeta measures
# Capture fraction types from file names in temp 'fraction_type' column and merge resulting dfs
fivex_abs_sol <- fivex_abs_sol_source %>% dplyr::mutate(fraction_type = 'Soluble')
fivex_abs_insol <- fivex_abs_insol_source %>% dplyr::mutate(fraction_type = 'Insoluble')
...
# Capture fraction types from file names in temp 'fraction_type' column and merge resulting dfs
nss_abs_sol_4mo <- nss_abs_sol_4mo_source %>% dplyr::mutate(fraction_type = 'Soluble')
nss_abs_sol_12mo <- nss_abs_sol_12mo_source %>% dplyr::mutate(fraction_type = 'Soluble')
nss_abs_insol_4mo <- nss_abs_insol_4mo_source %>% dplyr::mutate(fraction_type = 'Insoluble')
nss_abs_insol_12mo <- nss_abs_insol_12mo_source %>% dplyr::mutate(fraction_type = 'Insoluble')
...
# Abeta measures
# Capture fraction types from file names in temp 'fraction_type' column and merge resulting dfs
abca7_abs_sol_4mo <- abca7_abs_sol_4mo_source %>% dplyr::mutate(fraction_type = 'Soluble')
abca7_abs_sol_12mo <- abca7_abs_sol_12mo_source %>% dplyr::mutate(fraction_type = 'Soluble')
abca7_abs_insol_4mo <- abca7_abs_insol_4mo_source %>% dplyr::mutate(fraction_type = 'Insoluble')
abca7_abs_insol_12mo <- abca7_abs_insol_12mo_source %>% dplyr::mutate(fraction_type = 'Insoluble')
...
# compare generated file to latest version in synapse
```{r}
file_synID <- "syn61250724.11"
input_dir <- file.path("./", "input")

file_synID2 <- "syn61250724.12"
input_dir <- file.path("./", "input")

older_file <- download_csv(file_synID)
  path <- old_file$path

old_file <- download_csv(file_synID2)
  path <- old_file$path

# verify changes are expected
identical(old_file, older_file)

```

# manual genotype verification
```{r}
individualID <- 9733
nss_abs_sol_4mo[nss_abs_sol_4mo$individualID == individualID, ]
nss_individual[nss_individual$individualID == individualID, ]
nss_biospecimen[nss_biospecimen$individualID == individualID, ]
