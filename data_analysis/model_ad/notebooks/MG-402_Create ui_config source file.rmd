...
# Parent/child row keys
rna_de_row_id_data_key <- "composite_id"
rna_de_parent_id_data_key <- "composite_id"

# Display label noun
rna_de_view_noun <- list(singular = "Gene", plural = "Genes")
rna_de_parent_noun <- NA
...
rna_de_model_name_filter_vals_cerebral_cortex <- c(
                                               '5xFAD (UCI)',
                                               'Clu-h2kbKI',
                                               'Clu-h2kbKI.5xFAD')
...
# Display label nouns
protein_de_view_noun <- list(singular = "Protein", plural = "Proteins")
protein_de_parent_noun <- list(singular = "Gene", plural = "Genes")
...
# names, model filter values, leading/trailing column definitions, and view-level
# keys and display nouns) are passed in.
build_de_object <- function(menu1_label, tissue_label, dynamic_names,
                            ...
                            row_id_data_key, parent_id_data_key, view_noun, parent_noun = NULL) {
  ...
  list(
      ...
      row_id_data_key = row_id_data_key,
...
build_rna_de_objects <- function() {
  ...
  lapply(rna_de_menu2, function(tissue) {
    ...
    build_de_object(rna_de_menu1, tissue,
                    dynamic_names, dynamic_model_filter_vals,
                    leading_columns, trailing_columns,
                    rna_de_row_id_data_key, rna_de_parent_id_data_key,
                    rna_de_view_noun, rna_de_parent_noun)
  })
}
...
build_protein_de_objects <- function() {
  ...
  lapply(protein_de_menu2, function(tissue) {
    build_de_object(protein_de_menu1, tissue,
                    ...
                    leading_columns, trailing_columns,
                    protein_de_row_id_data_key, protein_de_parent_id_data_key,
                    protein_de_view_noun, protein_de_parent_noun)
  })
}
...
# Disease Correlation
# *******************
dc_page <- "Disease Correlation"
# when no noun is provided, app will fall back to display "Results"
dc_view_noun <- NA
dc_parent_noun <- NA
...
# ---------------------------------
dc_age_filter_vals <- c('4 months', '8 months', '12 months')
...
build_dc_objects <- function() {
  ...
  lapply(seq_len(nrow(dropdown_combos)), function(i) {
    ...
    list(
      ...
      view_noun = dc_view_noun,
      parent_noun = dc_parent_noun,
...
# ********************
mo_page <- "Mouse Model Overview"
mo_view_noun <- list(singular = "Model", plural = "Models")
mo_parent_noun <- NA

# Mouse Model Overview Columns
# ----------------------------
# name, type, data_key, tooltip, sort_tooltip, link_url, link_text, is_exported, is_hidden
...
# Mouse Model Overview Filter Values
# ----------------------------------
...
build_mo_object <- function() {
  ...
  list(
    page = mo_page,
    dropdowns = list(),
    view_noun = mo_view_noun,
    parent_noun = mo_parent_noun,
...
# ********************
marmo_mo_page <- "Marmoset Model Overview"
marmo_mo_view_noun <- list(singular = "Model", plural = "Models")
marmo_mo_parent_noun <- NA
marmo_ad_adkp_study_url <- "https://adknowledgeportal.synapse.org/Explore/Studies/DetailsPage/StudyDetails?Study=syn61849889"

# Marmo Model Overview Columns
# ----------------------------
# name, type, data_key, tooltip, sort_tooltip, link_url, link_text, is_exported, is_hidden
#
...
build_marmo_mo_object <- function() {
  ...
  list(
    page = marmo_mo_page,
    dropdowns = list(),
    view_noun = marmo_mo_view_noun,
    parent_noun = marmo_mo_parent_noun,
    row_count = model_overview_row_count,
    columns = columns,
    filters = filters
  )
