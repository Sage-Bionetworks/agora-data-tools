...
- **Tissue name mapping:** Maps `"Right Cerebral Hemisphere"` to `"Hemibrain"`, matching case-insensitively. Any other tissue passes through unchanged apart from surrounding whitespace. To add a mapping, add an entry to `TISSUE_ALIASES` in `rna_de_individual_utils.py`; `protein_de_individual` shares the same table.
...
- **Solution:** Update input data or add an entry to `TISSUE_ALIASES` in `rna_de_individual_utils.py`
