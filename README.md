...
* ```configs/agora_preprod.yaml``` places the transformed datasets in the [Agora Testing Data](https://www.synapse.org/#!Synapse:syn17015333) folder in synapse; write files to this folder to perform data validation.
* ```agora_prod.yaml``` places the transformed datasets the [Agora Live Data](https://www.synapse.org/#!Synapse:syn12177492) synapse folder; write files to this folder once you've validated that the ETL process is generating files suitable for release.
...
2. If the issue persists, it might be that `pipenv install` didn't install the dependencies correctly. To resolve this, follow these steps:
  ...
  - Verify the dependencies are installed properly:
    ...
    pipenv graph
    ```
    You should see a dependency graph generated by `pipenv`.

#### Shell for UNKNOWN_VIRTUAL_ENVIRONMENT already activated
If you encounter the following error when running `pipenv shell`:
```
...
Once you have completed the one-time setup step outlined above, execute the pipeline by running the following command and providing your PAT and the desired [config file](#config) as an argument. The following example command will execute the pipeline in Docker using ```configs/agora_preprod.yaml```:
