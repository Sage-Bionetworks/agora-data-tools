...
class TestMeltWideDataframe:
    """
    Test class for validating the melt_wide_dataframe utility function, which reshapes a
    wide assay matrix into long form and splits combined feature-column headers.
    """

    def test_basic_melt_and_header_split(self) -> None:
        """Test that value columns are melted and headers split into named parts."""
        df = pd.DataFrame(
            {
                "specimenid": ["s1", "s2"],
                "individualid": ["i1", "i2"],
                "Gene1|P00001": [1.0, 2.0],
                "Gene2|P00002": [3.0, 4.0],
            }
        )

        result = utils.melt_wide_dataframe(
            df,
            id_vars=["specimenid", "individualid"],
            header_names=["gene_symbol", "uniprotid"],
            value_name="value",
            sep="|",
        )
        ...
        assert len(result) == 4
        row = result[
            (result["individualid"] == "i1") & (result["uniprotid"] == "P00002")
        ].iloc[0]
        assert row["gene_symbol"] == "Gene2"
        assert row["value"] == 3.0

    def test_dropna_removes_unmeasured_values(self) -> None:
        """Test that rows with null values are dropped when dropna is True."""
        df = pd.DataFrame(
            {
                "individualid": ["i1", "i2"],
                "Gene1|P00001": [1.0, np.nan],
            }
        )
        ...
        assert len(result) == 1
        assert result.iloc[0]["individualid"] == "i1"

    def test_dropna_false_keeps_null_values(self) -> None:
        """Test that null values are retained when dropna is False."""
        df = pd.DataFrame(
            {
                "individualid": ["i1", "i2"],
                "Gene1|P00001": [1.0, np.nan],
    ...
    def test_empty_first_header_piece(self) -> None:
        """Test that a header with an empty leading piece splits without error.

        The 24-month proteomics file has headers like '|A0A240FAT1' where the gene symbol
        is absent; the uniprotid piece must still be captured correctly.
        """
        df = pd.DataFrame(
            {
                "individualid": ["i1"],
        ...
        assert result.iloc[0]["gene_symbol"] == ""
        assert result.iloc[0]["uniprotid"] == "A0A240FAT1"

    def test_separator_in_final_piece_is_preserved(self) -> None:
        """Test that only the first separator splits, so later separators stay in the tail."""
        df = pd.DataFrame(
            {
                "id": ["a"],
        ...
        result = utils.melt_wide_dataframe(
            df,
            id_vars=["id"],
            header_names=["first", "rest"],
        )

        assert result.iloc[0]["first"] == "part1"
        assert result.iloc[0]["rest"] == "part2|part3"

    def test_missing_id_var_raises_value_error(self) -> None:
        """Test that a missing id_vars column raises ValueError."""
        df = pd.DataFrame({"individualid": ["i1"], "Gene1|P00001": [1.0]})

        with pytest.raises(ValueError, match="id_vars columns not found"):
            utils.melt_wide_dataframe(
                df,
                id_vars=["specimenid"],
                header_names=["gene_symbol", "uniprotid"],
            )

    def test_empty_header_names_raises_value_error(self) -> None:
        """Test that empty header_names raises ValueError."""
        df = pd.DataFrame({"individualid": ["i1"], "Gene1|P00001": [1.0]})

        with pytest.raises(ValueError, match="header_names must not be empty"):
            utils.melt_wide_dataframe(
                df,
                id_vars=["individualid"],
                header_names=[],
...
class TestBucketAge:
    """
    Test class for validating the bucket_age utility function, which buckets continuous ages
    into nominal age-group labels using caller-supplied bin edges and labels.
    """

    bins = [float("-inf"), 6, 10, 16, 20, float("inf")]
    labels = ["4 months", "8 months", "12 months", "18 months", "24 months"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                6.0,
                "4 months",
            ),  # right-closed: value equal to edge falls in lower bucket
            (6.1, "8 months"),
            (10.0, "8 months"),
            (14.2, "12 months"),  # confirmed with data team
            (16.0, "12 months"),
            (18.0, "18 months"),
            (20.0, "18 months"),
            (24.2, "24 months"),
    ...
    def test_bucket_boundaries(self, age: float, expected: str) -> None:
        """Test that ages map to the correct right-closed bucket."""
        result = utils.bucket_age(pd.Series([age]), self.bins, self.labels)
        assert result.iloc[0] == expected

    def test_vectorized_series(self) -> None:
        """Test that a series of ages is bucketed elementwise."""
        result = utils.bucket_age(
            pd.Series([3.0, 12.1, 18.0, 24.2]), self.bins, self.labels
        )
    ...
    def test_value_outside_bins_becomes_none(self) -> None:
        """Test that ages outside every bin (including NaN) become None."""
        result = utils.bucket_age(
            pd.Series([5.0, float("nan")]),
            bins=[0, 6, 10],
            labels=["4 months", "8 months"],
        )
        assert result.iloc[0] == "4 months"
