...
class TestTransformRnaDeAggregate:
    ...
    def _mock_synapse_data(self, mock_data_files):
        ...
        def mock_get_entity_as_df(syn_id, source, syn):
            ...
            if syn_id == "syn123456":
                return pd.read_csv(
                    os.path.join(self.data_files_path, "input", "test_data_1.csv")
                )
            elif syn_id == "syn789012":
                return pd.read_csv(
                    os.path.join(self.data_files_path, "input", "test_data_2.csv")
                )
            else:
    ...
    def test_transform_rna_de_aggregate_should_pass(
        ...
        with mock.patch(
            "agoradatatools.etl.transform.rna_de_aggregate._login_to_synapse"
        ) as mock_login, mock.patch(
            "agoradatatools.etl.transform.rna_de_aggregate.get_entity_as_df"
        ) as mock_get_entity:
    ...
    def test_transform_rna_de_aggregate_empty_data_file(self):
        ...
        datasets = {
            "rna_de_aggregate_data_files": pd.DataFrame(
                {"file_name": ["test_data_empty.csv"], "syn_id": ["syn_empty"]}
            ),
        ...
        def mock_get_entity_as_df_empty(syn_id, source, syn):
            if syn_id == "syn_empty":
                return pd.read_csv(
                    os.path.join(self.data_files_path, "input", "test_data_empty.csv")
                )
            else:
                raise ValueError(f"Unknown syn_id: {syn_id}")
    ...
    def test_transform_rna_de_aggregate_missing_columns_in_data_file(self):
        ...
        def mock_get_entity_as_df_missing(syn_id, source, syn):
            if syn_id == "syn_missing":
                return pd.read_csv(
                    os.path.join(
                        self.data_files_path, "input", "test_data_missing_columns.csv"
                    )
                )
            else:
                raise ValueError(f"Unknown syn_id: {syn_id}")
    ...
    def test_transform_rna_de_aggregate_human_genes_filtered(self):
        ...
        datasets = {
            "rna_de_aggregate_data_files": pd.DataFrame(
                {"file_name": ["test_data_mixed_genes.csv"], "syn_id": ["syn_mixed"]}
            ),
            "rnaseq_genotype_label_map": pd.DataFrame(
                {
                    "model": ["APP/PS1"],
    ...
    def test_transform_rna_de_aggregate_jax_tissue_mapping(self):
        ...
        datasets = {
            "rna_de_aggregate_data_files": pd.DataFrame(
                {"file_name": ["test_data_jax.csv"], "syn_id": ["syn_jax"]}
            ),
            "rnaseq_genotype_label_map": pd.DataFrame(
                {
                    "model": ["JAX_model"],
    ...
    def test_transform_rna_de_aggregate_age_sorting(self):
        ...
        datasets = {
            "rna_de_aggregate_data_files": pd.DataFrame(
                {"file_name": ["test_data_age_sorting.csv"], "syn_id": ["syn_age"]}
            ),
            "rnaseq_genotype_label_map": pd.DataFrame(
                {
                    "model": ["APP/PS1"],
